    ...
    return (
        <div className='flex justify-between gap-x-2 '>
            <div className='flex  gap-x-2'>
                <div>
                    <input onChange={e => setSearchInput(e.target.value)} value={searchInput} type="text" placeholder="Search Transactions" className="input input-bordered w-full max-w-xs !outline-none " />
                ...
                <div className="btn-group ">
                    <button disabled={currentPage===1} onClick={()=>setCurrentPage(currentPage-1)} className="btn">«</button>
                    <button className="btn">Page {currentPage}/{totalPage || 1}</button>
                    <button disabled={currentPage===totalPage} onClick={()=>setCurrentPage(currentPage+1)} className="btn">»</button>
                </div>
